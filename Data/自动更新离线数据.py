...
log: IO[Any] = open(f"Data/Update Logs/{currentTime}.log", "w", encoding = "utf-8")
...
while True:
    ...
    if resource == "":
        ...
        while True:
            if step == 1:
                ...
                if option != "" and option[0] == "0":
                    step -= 1
                    continue
                elif option != "" and option[0] in {"1", "2", "3", "5"}:
                    option = option[0]
                else:
                    option = "1"
            elif step == 2:
        ...
        if option == "4":
            ...
            source, status, session = requestUrl("GET", "https://raw.communitydragon.org/latest/plugins/rcp-be-lol-game-data/global/", session)
            if status != 200:
                if status == -1:
            ...
            source, status, session = requestUrl("GET", "https://raw.communitydragon.org/pbe/plugins/rcp-be-lol-game-data/global/", session)
            if status != 200:
                if status == -1:
            ...
            if input_method != "" and input_method[0] == "2":
                ...
                while True:
                    ...
                    if cdragon_folder == "":
                        ...
                        cdragon_folders.append(cdragon_folder.replace("https://raw.communitydragon.org/", "").replace("Data/cdragon/", "")) #请思考，这里如果换成`cdragon_folder.lstrip("https://raw.communitydragon.org/")`，会有什么效果？（Please figure out what will happen if the code is replaced by `cdragon_folder.lstrip("https://raw.communitydragon.org/")`）
        ...
        for folder in cdragon_folders:
            cnt1 += 1
            cnt2: int = 0
            url: str = urljoin(web_prefix, folder) #在查验文件夹时，这个变量不再直接传入网址请求函数中了。它只作为输出提示（This variable is no longer passed to the `requestUrl` function when checking the folder. It's only used for prompts now）
            ...
            if delete:
                for file in files_to_delete:
                    try:
                        ...
                        pass
            logPrint("", write_time = False)
        if option in {"1", "2", "3"} and complete_scan and len(folders_to_delete):
        ...
        while True:
            if step == 1:
                ...
                if ddragon_hint:
                    hint: str = '请按以下步骤操作：\nPlease follow these steps:\n1. 访问网址https://developer.riotgames.com/docs/lol#data-dragon\n   Visit the website: https://developer.riotgames.com/docs/lol#data-dragon\n2. 在Latest中找到正式服最新版本数据资源压缩包下载链接。例如：https://ddragon.leagueoflegends.com/cdn/dragontail-16.9.1.tgz\n   Find the link to download the compressed tarball of the latest data resource for live servers. For example: https://ddragon.leagueoflegends.com/cdn/dragontail-16.9.1.tgz\n3. 下载。这需要花费一些时间。\n   Download the file. It may take some time.\n4. 将下载好的tgz文件直接“解压至此”。\n   "Extract here" for the tgz file.\n5. 将解压出来的压缩包再次解压到选定文件夹下与压缩包同名的文件夹。示例：将“dragontail-16.9.1.tar”解压到“D:/360AI浏览器下载/dragontail-16.9.1”文件夹下。\nExtract to "Archive-Name" folder under the selected folder for the extracted tar file. For example, extract "dragontail-16.9.1.tar" into the folder "D:/Downloads/dragontail-16.9.1".\n接下来，请给出数据资源的位置。（按照上例应为“D:/360AI浏览器下载/dragontail-16.9.1/16.9.1/data”。）\nNext, please provide the directory that stores the data resources. (By the above example, the directory should be "D:/Downloads/dragontail-16.9.1/16.9.1/data".)'
        ...
        src_json: Any = source.json()
        src: str = json.dumps(src_json, indent = 4, ensure_ascii = False)
        if not "versions.json" in os.listdir("Data"):
            update = added = True
        else:
            with open("Data/versions.json", "r", encoding = "utf-8") as fp:
                dst: str = fp.read()
            if src != dst:
                update = True
        if update:
            with open("Data/versions.json", "w", encoding = "utf-8") as fp:
                fp.write(src)
            if added:
                logPrint("[%d]已添加文件（Added file）：Data/versions.json" %cnt1, print_time = True)
                added_files.append("Data/versions.json")
            else:
                logPrint("[%d]已更新文件（Updated file）：Data/versions.json" %cnt1, print_time = True)
                updated_files.append("Data/versions.json")
    else:
